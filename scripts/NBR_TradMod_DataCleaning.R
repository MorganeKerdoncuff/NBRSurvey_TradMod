...
siteinfo_raw[is.na(siteinfo_raw$Municipality),] # NA identified - Stordalen is located in Masfjorden

unique(Site_Info_raw$Farmer) #needs full name Dan
#Site_Info_raw[is.na(Site_Info_raw$Farmer)]
#unique(Site_Info_raw$Type_livestock)
Site_Info_raw <- Site_Info_raw %>% 
  mutate(Type_livestock = dplyr::recode(Type_livestock, "Villsau" = "Sheep")) # Villsau as sheep category
table(Site_Info_raw$Type_livestock) #10 cows, 11 goats, 23 sheep -> ask Amy again about UG1 & UG2
#unique(Site_Info_raw$Habitat) #needs to standardise categories, no more than 5 - burnt/non-burnt? alpine/subalpine? 
#unique(Site_Info_raw$Animal_on_site)
